...
const PRIORITY_LABELS = { high: 'Срочно', med: 'Средний', '': 'Низкий' }
const PRIORITY_CLS = { high: 'tp-h', med: 'tp-m' }
  ...
  async function save() {
    ...
    try {
      if (modal.type === 'stage') {
        ...
        await api.post('/stages/task', { ...form, project_id: id, stage_id: modal.stage_id, substage_id: modal.substage_id || null })
      }
      await loadStages(); setModal(null); setForm({})
    ...
    <div>
      ...
      <div className="panel" style={{ marginBottom: 14, padding: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1.5, color: 'var(--navy)' }}>Общий прогресс проекта</div>
          <div style={{ fontSize: 20, fontWeight: 700, fontFamily: "'Bebas Neue',sans-serif", color: 'var(--navy)' }}>{totalProgress}%</div>
        </div>
      ...
      {stages.length === 0 && (
        <div className="panel" style={{ padding: 24, textAlign: 'center' }}>
          ...
          {isAdmin && (
            <button className="btn" onClick={addDefaults}><i className="ti ti-template" /> Добавить стандартные этапы</button>
          )}
        </div>
      )}
          ...
          <div style={{ padding: '12px 14px', borderBottom: expanded[`s${stage.id}`] ? '1px solid var(--border)' : 'none', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => toggle(`s${stage.id}`)}>
            ...
            {isAdmin && (
              <div style={{ display: 'flex', gap: 4 }} onClick={e => e.stopPropagation()}>
                ...
                <button className="btn-sec" style={{ padding: '3px 7px', fontSize: 11 }} onClick={() => { setModal({ type: 'task', stage_id: stage.id }); setForm({ text: '', priority: 'med' }) }}>
          ...
          {expanded[`s${stage.id}`] && (
            <div>
              ...
              {stage.tasks.map(task => (
                <TaskRow key={task.id} task={task} onToggle={toggleTask} onDelete={t => del('task', t)} isAdmin={isAdmin} />
              ))}
              ...
              {stage.substages.map(sub => (
                <div key={sub.id} style={{ borderTop: '1px solid var(--border)' }}>
                  <div style={{ padding: '10px 14px 10px 28px', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', background: '#fafaf8' }} onClick={() => toggle(`sb${sub.id}`)}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)' }}>{sub.name}</div>
                    ...
                    {isAdmin && (
                      <div style={{ display: 'flex', gap: 4 }} onClick={e => e.stopPropagation()}>
                        <button className="btn-sec" style={{ padding: '2px 6px', fontSize: 11 }} onClick={() => { setModal({ type: 'task', stage_id: stage.id, substage_id: sub.id }); setForm({ text: '', priority: 'med' }) }}>
                  ...
                  {expanded[`sb${sub.id}`] && (
                    <div>
                      {sub.tasks.map(task => (
                        <TaskRow key={task.id} task={task} onToggle={toggleTask} onDelete={t => del('task', t)} isAdmin={isAdmin} indent />
                      ))}
                      {sub.tasks.length === 0 && (
                        <div style={{ padding: '8px 14px 8px 42px', fontSize: 11, color: 'var(--muted)' }}>Нет задач</div>
                      )}
                    </div>
                  )}
                </div>
            ...
            <div className="mh">
              <div className="mh-title">
                {modal.type === 'stage' ? (modal.id ? 'Редактировать этап' : 'Новый этап') :
                 modal.type === 'substage' ? (modal.id ? 'Редактировать подэтап' : 'Новый подэтап') :
                 'Новая задача'}
              </div>
            ...
            <div className="mb">
              ...
              {modal.type === 'task' && (
                <>
                  ...
                  <div className="fg"><label>Исполнитель</label>
                    <select value={form.assigned_to || ''} onChange={e => { set('assigned_to', e.target.value); set('assigned_name', users.find(u => u.id === Number(e.target.value))?.name || '') }}>
    ...
    <div className="ti" style={{ paddingLeft: indent ? 42 : 28, borderTop: '1px solid var(--border)' }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 2, flexWrap: 'wrap' }}>
          ...
          {task.assigned_name && <span style={{ fontSize: 10, color: 'var(--navy)', background: '#eef0fa', padding: '1px 6px' }}>{task.assigned_name}</span>}
        </div>
      </div>
