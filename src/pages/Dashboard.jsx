...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    api.get('/finance').then(r => setFinance(r.data)).catch(() => {})
  ...
  return (
    <div>
      <div className="kpi">
        ...
        <div className="kc"><div className="kl">Задач открыто</div><div className="kv">{pendingTasks.length}</div><div className="ks">{pendingTasks.filter(t => t.due && t.due < today).length > 0 ? <span className="dn">⚠ есть просроченные</span> : 'в работе'}</div></div>
      </div>
      <div className="two">
        <div className="panel">
